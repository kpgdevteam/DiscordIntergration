...
namespace DiscordIntegration.API.Commands
{
    ...
    public class PlayerCommandSender : CommandSender
    {
        ...
        /// <inheritdoc/>
        public override void Print(string text) => player.GetSender().Print(text);

        /// <inheritdoc/>
        public override void RaReply(string text, bool success, bool logToConsole, string overrideDisplay) => player.GetSender().RaReply($"DISCORDINTEGRATION#{text}", success, logToConsole, overrideDisplay);
    }
}
